"""
Transformer and Attention Components for SVoRT

This module provides the building blocks for the SVoRT transformer architecture:
- Multi-head self-attention mechanisms
- Positional encoding for slice/transformation information
- ResNet-based feature extraction
- Sparse attention variants (entmax, sparsemax)

Key Components:
--------------
1. **Attention Layers**:
   - MultiHeadAttention: Standard scaled dot-product attention with multiple heads
   - ScaledDotProductAttention: Core attention computation with optional sparse variants
   - Supports softmax, entmax15, and sparsemax activations

2. **Feed-Forward Layers**:
   - PositionwiseFeedForward: Two-layer MLP with configurable activation
   - Pre-norm or post-norm variants for better training stability

3. **Encoder Architecture**:
   - EncoderLayer: Single transformer encoder layer (attention + FFN)
   - TransformerEncoder: Stack of encoder layers with positional encoding
   - Supports prenorm (LayerNorm before sublayers) for stability

4. **Positional Encoding**:
   - PositionalEncoding: Sinusoidal encoding for continuous features
   - Encodes slice positions and transformation parameters
   - Learnable frequency bands for different feature types

5. **Feature Extraction**:
   - ResNet: Pretrained or random-init ResNet for image features
   - Flexible input channels (grayscale MRI or multi-channel)
   - Produces per-slice feature vectors for transformer

6. **Sparse Attention** (optional):
   - Sparsemax: Projects attention to sparse simplex (some weights = 0)
   - Entmax (α=1.5): Intermediate between softmax and sparsemax
   - Useful for interpretability and computational efficiency

Architecture Notes:
------------------
- Pre-norm (prenorm=True) generally more stable for deep networks
- Dropout applied after attention and FFN for regularization
- Attention masks can enforce causal or local constraints
- ResNet frozen or fine-tuned depending on data availability

References:
----------
- Vaswani et al. "Attention Is All You Need" (NeurIPS 2017)
- Peters et al. "Entmax: Sparse attention" (ICML 2019)
- Code adapted from: https://github.com/jadore801120/attention-is-all-you-need-pytorch
"""
